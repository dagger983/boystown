...
const AnnualDay = () => {
  ...
  useEffect(() => {
    const fetchEventData = async () => {
      ...
      try {
        ...
        // Filter the data to include only events with the category "annual-day"
        const annualDayEvents = data.filter(event => event.category === 'annual-day');
        setEventData(annualDayEvents);

        // Extract unique years from the filtered data
        const uniqueYears = [...new Set(annualDayEvents.map(event => event.year))];
  ...
  const handleYearChange = (e) => {
    ...
    // Find the event details for the selected year and category "annual-day"
